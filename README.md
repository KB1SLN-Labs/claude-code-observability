...
Most Codex panels are built from **logs** (conversations, tokens, tools, decisions), so the logs exporter alone covers the bulk of the dashboard. Adding the **traces** exporter feeds the trace-derived operation-latency panels (and Grafana's Tempo explorer) — these come from CLI traces, so they work without the desktop. The **metrics** exporter is only emitted by the interactive/desktop app (see the note below); it isn't required for the shipped panels. Codex's telemetry carries no per-request dollar cost, so the dashboard tracks **usage and performance**, not spend.
...
Use the full `env` block from [Configuring Claude Code](#configuring-claude-code) (the enable switch and per-signal exporters are what actually turn telemetry on). Only `OTEL_EXPORTER_OTLP_ENDPOINT` changes per deployment:

- **Same machine as Claude Code:** `"OTEL_EXPORTER_OTLP_ENDPOINT": "http://localhost:4318"`
- **Stack on a remote host:** `"OTEL_EXPORTER_OTLP_ENDPOINT": "http://<stack-host>:4318"` (the IP/hostname of the Docker machine)
...
See [Configuring Claude Code](#configuring-claude-code) for the full `env` block. Set `OTEL_EXPORTER_OTLP_ENDPOINT` to the collector's external IP: `"http://<otel-collector-external-ip>:4318"`.
...
Prometheus, Loki, and Tempo each get a 10Gi PersistentVolumeClaim by default. Prometheus is configured to retain 30 days of metrics; Tempo retains 7 days of traces. Adjust PVC sizes in `k8s/prometheus.yaml`, `k8s/loki.yaml`, and `k8s/tempo.yaml` before first deploy.
...
| `tempo.retention` | `168h` | How long Tempo keeps traces (7 days) |
| `tempo.persistence.size` | `10Gi` | Tempo PVC size |
...
| `tempo.persistence.storageClass` | `""` | StorageClass for Tempo PVC (cluster default if empty) |
...
- **Configuration changes for traces:** to capture traces, point each agent's trace exporter at the collector. For **Codex**, add the metrics/traces exporters to `config.toml` (see [Configuring Codex](#configuring-codex)). For **Claude Code**, add `OTEL_TRACES_EXPORTER=otlp` and the `CLAUDE_CODE_ENHANCED_TELEMETRY_BETA=1` beta flag (see [Configuring Claude Code](#configuring-claude-code)). Metrics and logs for both agents are unaffected.
...
**Trace panels or the Tempo explorer are empty**

Traces are off by default and need explicit enabling. Confirm the agent's trace exporter is configured (`OTEL_TRACES_EXPORTER=otlp` + `CLAUDE_CODE_ENHANCED_TELEMETRY_BETA=1` for Claude Code; the `[otel.trace_exporter.otlp-http]` block for Codex) and that the agent was fully restarted afterward. Then check the collector is forwarding to Tempo (it logs no `otlp/tempo` export errors) and that Tempo's pod/container is healthy. The trace-derived **p95** panels also need a few turns within their window before a percentile can be computed — they read empty during idle.
...
Four dashboards ship with the stack: **Claude Code** (main) and **Claude Code — Logs**, plus **Codex** (main) and **Codex — Logs**. They're cross-linked via header links so you can jump between them.
